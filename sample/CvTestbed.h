...
protected:
	...
	static void default_videocallback(IplImage *image, struct timeval* timestamp,bool dis);
...
public:
	 ...
	 *        every frame.
	 */
	void SetVideoCallback(void (*_videocallback)(IplImage *image,struct timeval* timestamp));
	/** 
	...
	bool ToggleImageVisible(size_t index, int flags=1);
        void RecordImages(bool rec);
        void RecordVideo(bool rec);
        void SetRecordDirectory(std::string dir);

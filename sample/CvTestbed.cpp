...
*/
		...
		if( (key = cvWaitKey(1)) >= 0 ) {
			...
			// By default '0'-'9' toggles visible images
				...
				//Ben Addition to use keyboard as command input
				//update control class command key
				ControlClass::Instance().SetCmdKey(key);
				if (key == 'q'){
				ControlClass::Instance().SetCmdKey('t'); //kill the motors
